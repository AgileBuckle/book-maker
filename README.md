...
Instead of setting every book's type by hand, you can upload a CSV with the book name in column A, the spine type (Perfect bound, Hardcover, or Saddlestitch) in column B, and — optionally, for hardcover rows only — a hex back-cover color (e.g. `#3d2b1a`) in column C. Book names are fuzzy-matched to your uploaded covers, so the CSV doesn't need exact filenames, and a header row is detected automatically and skipped. Leave column C blank for non-hardcover rows, or for hardcover rows where you want the automatic color guess left alone.

### Running a Batch

Once your books are ready — each one either doesn't need a spine, or has a spine matched to it, with hardcover back colors resolved — click "Start Batch" to generate them one at a time. When finished, a `book-maker-batch.zip` downloads containing a PNG for every book plus a `book-maker-batch.csv` recording each book's type (and, for hardcover books, its back-cover color). Re-upload that CSV on a future batch (as the Spine Type CSV above) to redo the same run without resetting every book's settings by hand. If an image fails to load partway through, the batch stops and tells you which file caused the problem.
